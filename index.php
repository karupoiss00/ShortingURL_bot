    ...
    if($text){
        if ($text == '/start') {
            if (strlen($name) == 0) {
                ...
                $reply = 'Добро пожаловать, '.$name.'! Для ознакомления с функционалом, воспользуйтесь командой /help.';
            }
            $telegram->sendMessage([ 'chat_id' => $chat_id, 'text' => $reply]);
        }
        elseif ($text == '/help') {
            $reply = 'Данный бот предназначен для работы со ссылками. Он умеет сокращать или расшифровывать уже сокращённые ссылки. Для того, чтобы воспользоваться ботом, отправьте ему ссылку, которую необходимо сократить или расшифровать.';
            $telegram->sendMessage([ 'chat_id' => $chat_id, 'text' => $reply ]);
        }
        else {
